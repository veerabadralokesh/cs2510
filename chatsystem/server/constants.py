
import os

NUM_SERVERS = 5
SERVER_IDS = [str(i) for i in range(1, NUM_SERVERS+1)]

REPLAY_MSG_LOG_INTERVAL_MICROS = 1_000_000 * 10

STORE_DATA_ON_FILE_SYSTEM = True

DATA_STORE_FILE_DIR_PATH = os.path.join(os.path.expanduser('~'), 'chat_server_data', 'chat_server{}')

# os.makedirs(DATA_STORE_FILE_DIR_PATH, exist_ok=True)

DATA_STORE_FILE_PATH = os.path.join(DATA_STORE_FILE_DIR_PATH, 'server_datastore.json')

CONNECTION_COMMANDS = ['c']
LOGIN_COMMANDS = ['u']
JOIN_GROUP_COMMANDS = ['j']
EXIT_APP_COMMANDS = ['q']
APPEND_TO_CHAT_COMMANDS = ['a']
LIKE_COMMANDS = ['l']
UNLIKE_COMMANDS = ['r']
NEW = 'new'
USER_JOIN = 'joined'
USER_LEFT = 'left'
PARTICIPANT_LIST = 'PARTICIPANT_LIST'

MESSAGE_UPDATE_INTERVAL = 50

CONNECT_SERVER_INTERVAL = 1
PING_INTERVAL = 1
PING_TIMEOUT = 0.2

DELETE_MESSAGE_FROM_DISK_INTERVAL = 5

USE_DIFFERENT_PORTS = os.getenv('USE_DIFFERENT_PORTS', '1') == '1'

SERVER_STRING = '172.30.100.10{}:12000'

MESSAGE_EVENT = 'MESSAGE_EVENT'
GROUP_EVENT = 'GROUP_EVENT'
GROUP_META_DATA = 'GROUP_META_DATA'
GET_GROUP_META_DATA = 'GET_GROUP_META_DATA'


CHANGE_LOG_APPEND = 'APPEND'
CHANGE_LOG_INSERT = 'INSERT'
CHANGE_LOG_UPDATE = 'UPDATE'
CHANGE_LOG_USERS_UPDATE = 'USERS_UPDATE'

NEGATIVE_MESSAGE_INDEX = 'NEGATIVE_MESSAGE_INDEX'

SERVER_DIED_CALLBACK = 'SERVER_DIED_CALLBACK'
GET_VECTOR_TIMESTAMP = 'GET_VECTOR_TIMESTAMP'
